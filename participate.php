...
if(isset($_POST['like_content'])){
   ...
   if($user_id != ''){
      ...
      $select_question = $conn->prepare("SELECT * FROM `questions`");
      $select_question->execute([$content_id]);
      $fetch_question = $select_question->fetch(PDO::FETCH_ASSOC);

      $tutor_id = $fetch_question['user_id'];
...
if(isset($_POST['add_comment'])){
   ...
   if($user_id != ''){
      ...
      $select_question = $conn->prepare("SELECT * FROM `question` WHERE id = ? LIMIT 1");
      $select_question->execute([$content_id]);
      $fetch_question = $select_question->fetch(PDO::FETCH_ASSOC);

      $tutor_id = $fetch_question['tutor_id'];
...
?>
    ...
    <section class="watch-video">
        ...
        <?php
      $select_question = $conn->prepare("SELECT * FROM `questions`");
      $select_question->execute([$get_id, 'active']);
      if($select_question->rowCount() > 0){
         while($fetch_question = $select_question->fetch(PDO::FETCH_ASSOC)){
            $content_id = $fetch_question['id'];
            ...
            $select_tutor->execute([$fetch_question['tutor_id']]);
            $fetch_tutor = $select_tutor->fetch(PDO::FETCH_ASSOC);
   ?>
        <div class="video-details">
            <video src="uploaded_files/<?= $fetch_question['video']; ?>" class="video"
                poster="uploaded_files/<?= $fetch_question['thumb']; ?>" controls autoplay></video>
            <h3 class="title"><?= $fetch_question['title']; ?></h3>
            <div class="info">
                <p><i class="fas fa-calendar"></i><span><?= $fetch_question['date']; ?></span></p>
            ...
            <form action="" method="post" class="flex">
                ...
                <a href="playlist.php?get_id=<?= $fetch_question['playlist_id']; ?>" class="inline-btn">view
                    playlist</a>
                <?php
         ...
         ?>
            ...
            <div class="description">
                <p><?= $fetch_question['description']; ?></p>
            </div>
        </div>
        <?php
         }
